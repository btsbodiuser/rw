...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    const savedCart = localStorage.getItem('rw-cart');
    if (savedCart) {
      setCart(JSON.parse(savedCart));
    }
    const savedRecentlyViewed = localStorage.getItem('rw-recently-viewed');
    if (savedRecentlyViewed) {
      setRecentlyViewed(JSON.parse(savedRecentlyViewed));
    }
    // Restore auth from token
    const savedToken = localStorage.getItem('rw-auth-token');
    if (savedToken) {
  ...
  useEffect(() => {
    localStorage.setItem('rw-cart', JSON.stringify(cart));
  }, [cart]);

  // Save recently viewed to localStorage
  useEffect(() => {
    localStorage.setItem('rw-recently-viewed', JSON.stringify(recentlyViewed));
  }, [recentlyViewed]);
  ...
  const login = async (identifier: string, password: string): Promise<boolean> => {
    try {
      ...
      localStorage.setItem('rw-auth-token', result.token);
  ...
  const clearRecentlyViewed = useCallback(() => {
    setRecentlyViewed([]);
    localStorage.removeItem('rw-recently-viewed');
  }, []);
