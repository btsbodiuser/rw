...
export const ProductEntryPage = () => {
  ...
  const [cargoIncludedInPrice, setCargoIncludedInPrice] = useState(false);
  ...
  useEffect(() => {
    ...
    fetchProductForEdit(authToken, editId)
      .then((p) => {
        ...
        setCargoIncludedInPrice(!!p.cargo_included_in_price);
        if (p.main_image) {
          setMainImage({
            id: p.main_image.id,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (type === 'preorder' && (!weightKg || parseFloat(weightKg) <= 0) && !cargoIncludedInPrice) {
      toast.error('Урьдчилсан захиалгын барааны жин оруулна уу, эсвэл "Тээврийн зардал үнэнд орсон" сонголтыг тэмдэглэнэ үү');
      return;
    }
    ...
    try {
      ...
      payload.cargo_included_in_price = cargoIncludedInPrice ? 1 : 0;
          ...
          {type === 'preorder' && (
            <div className="flex items-center gap-3">
              <label className="relative inline-flex items-center cursor-pointer">
                <input
                  type="checkbox"
                  checked={cargoIncludedInPrice}
                  onChange={(e) => setCargoIncludedInPrice(e.target.checked)}
                  className="sr-only peer"
                />
              ...
              <div>
                <span className="text-sm font-medium text-gray-700">Тээврийн зардал үнэнд орсон</span>
                <p className="text-xs text-gray-500">Идэвхтэй бол энэ барааны ачааны төлбөр тооцохгүй (жин заавал биш)</p>
              </div>
            </div>
          )}
