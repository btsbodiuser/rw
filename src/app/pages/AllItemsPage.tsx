...
export const AllItemsPage = () => {
  ...
  const initialFilter = typeParam === 'preorder' ? 'preorder' : (typeParam === 'ready' ? 'ready' : (preorderOnly ? 'preorder' : 'all'));
  const initialSort: 'newest' | 'popular' | 'price-low' | 'price-high' =
    sortParam === 'price-low' || sortParam === 'price-high' || sortParam === 'popular' || sortParam === 'newest'
      ? sortParam
      : 'newest';
  ...
  const [sortBy, setSortBy] = useState<'newest' | 'popular' | 'price-low' | 'price-high'>(initialSort);
  ...
  useEffect(() => {
    ...
    if (typeParam === 'preorder') setFilter('preorder');
    if (sortParam === 'newest' || sortParam === 'popular' || sortParam === 'price-low' || sortParam === 'price-high') {
      setSortBy(sortParam);
    }
  }, [searchParams]);
  ...
  const sortMap = { 'newest': 'newest', 'popular': 'popular', 'price-low': 'price_asc', 'price-high': 'price_desc' } as const;
  ...
  const applyTypeFilter = (nextFilter: 'all' | 'ready' | 'preorder') => {
    setFilter(nextFilter);
    const params = new URLSearchParams(searchParams);
    params.delete('preorder'); // normalize legacy flag into type
    if (nextFilter === 'all') {
      params.delete('type');
    } else {
    ...
    navigate(qs ? `/all-items?${qs}` : '/all-items', { replace: true });
  };
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button
            onClick={() => applyTypeFilter('preorder')}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
              filter === 'preorder'
                ? 'bg-blue-600 text-white'
                : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-6">
        <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-2">
          {searchText ? `"${searchText}" хайлтын үр дүн` : ((typeParam === 'preorder' || preorderOnly) ? 'Урьдчилсан захиалга' : (typeParam === 'ready' ? 'Бэлэн бараа' : 'Бүх бүтээгдэхүүн'))}
        </h1>
