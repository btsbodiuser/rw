...
export const CheckoutPage = () => {
  ...
  const hasCargoItems = cart.some(item => item.product.type === 'preorder' && item.product.weightKg && !item.product.cargoIncludedInPrice && settings.cargo_rate_per_kg > 0);

  const cargoFee = cart.reduce((sum, item) => {
    if (item.product.type === 'preorder' && item.product.weightKg && !item.product.cargoIncludedInPrice && !item.product.hideCargoFee) {
  ...
  useEffect(() => {
    const saved = sessionStorage.getItem('guzeelzgene-pending-payment');
  ...
  const hasPendingPayment = restored || !!sessionStorage.getItem('guzeelzgene-pending-payment');
  ...
  useEffect(() => {
    if (step === 'payment' && orderNumber) {
      sessionStorage.setItem('guzeelzgene-pending-payment', JSON.stringify({ qpayInvoiceId, bonumInvoiceId, bonumFollowUpLink, storepayInvoiceId, storepayMobile, orderNumber, total, confirmedTotal: confirmedTotal ?? total, qrCodeUrl, qpayUrls, paymentMethod, cartFingerprint }));
    }
    if (step === 'complete') {
      sessionStorage.removeItem('guzeelzgene-pending-payment');
    }
  ...
  const handleSwitchPaymentMethod = async (newMethod: 'qpay' | 'transfer' | 'bonum' | 'storepay') => {
    ...
    try {
      ...
      if (newMethod === 'qpay') {
        ...
        try {
          const invoice = await createBonumInvoice(orderNumber, confirmedTotal ?? total, `Guzeelzgene захиалга #${orderNumber}`);
    ...
    try {
      ...
      if (paymentMethod === 'qpay') {
        try {
          ...
          setQrCodeUrl(`https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=guzeelzgene_${currentOrderNumber}_${currentTotal}`);
          setStep('payment');
        }
        ...
        try {
          const invoice = await createBonumInvoice(
            currentOrderNumber,
            currentTotal,
            `Guzeelzgene захиалга #${currentOrderNumber}`
          );
          setBonumInvoiceId(invoice.invoice_id);
          setBonumFollowUpLink(invoice.follow_up_link);
          setStep('payment');
              ...
              {cart.map((item) => (
                <div key={item.product.id} className="flex gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {item.product.type === 'preorder' && item.product.weightKg && !item.product.cargoIncludedInPrice && settings.cargo_rate_per_kg > 0 && (
            ...
            <div className="border-t pt-4 space-y-2">
              ...
              {hasCargoItems && (
                <>
                  ...
                  {cart.some(item => item.product.type === 'preorder' && item.product.weightKg && !item.product.cargoIncludedInPrice && item.product.hideCargoFee) && (
                    <div className="flex justify-between text-orange-700">
                      <span>Ачааны төлбөр *</span>
