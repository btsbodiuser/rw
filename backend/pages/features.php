...
$items = $db->query("SELECT * FROM features ORDER BY sort_order, id")->fetchAll();
require_once __DIR__ . '/../includes/header.php';
?>

<div class="flex items-center justify-between mb-6">
    <p class="text-sm text-gray-500"><?= count($items) ?> хайрцаг · /shop хуудсанд харагдана</p>
    <a href="index.php?page=feature-form"
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    <table class="w-full">
        <thead class="bg-gray-50 border-b border-gray-100">
            <tr>
                <th class="text-left px-5 py-3 text-xs font-medium text-gray-500 uppercase">Icon</th>
        ...
        <tbody class="divide-y divide-gray-50">
            <?php if (empty($items)): ?>
                <tr><td colspan="6" class="px-5 py-10 text-center text-gray-400">Хайрцаг байхгүй байна</td></tr>
            <?php endif; ?>
            <?php foreach ($items as $item): ?>
            <tr class="hover:bg-gray-50">
                <td class="px-5 py-3 text-xs font-mono text-gray-500"><?= e($item['icon']) ?></td>
                <td class="px-5 py-3 font-medium text-gray-900"><?= e($item['title_mn']) ?></td>
                <td class="px-5 py-3 text-sm text-gray-500"><?= e($item['description_mn'] ?? '') ?></td>
                <td class="px-5 py-3 text-center text-sm text-gray-600"><?= (int)$item['sort_order'] ?></td>
                <td class="px-5 py-3 text-center">
                    <span class="w-2.5 h-2.5 rounded-full inline-block <?= $item['is_active'] ? 'bg-green-500' : 'bg-gray-300' ?>"></span>
                </td>
                <td class="px-5 py-3 text-right">
                    <div class="flex items-center justify-end gap-2">
                        <a href="index.php?page=feature-form&id=<?= (int)$item['id'] ?>"
                        ...
                        <form method="POST" action="index.php?page=feature-delete" class="inline"
                              onsubmit="return confirm('«<?= e(addslashes($item['title_mn'])) ?>» устгах уу?')">
                            <?= csrfField() ?>
